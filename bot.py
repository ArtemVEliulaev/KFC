...
all_text = {"номер_т_кн": {"ru":'📞Поделиться номером телефона📞', "uz":'📞Поделиться номером телефона📞'},
            ...
            "нов_заказ": {"ru":'Новый заказ от пользователя :',"uz":"Foydalanuvchidan yangi buyurtma :"},
...
def get_name(message,language):
    ...
    name = message.text
    print(name)
    bot.send_message(user_id, text = all_text.get("номер_т").get(language),
                     reply_markup=bt.phone_number_bt(user_id))
...
def get_phone(message, name,language):
    ...
    if message.contact:
        ...
        print(phone_number)
        bot.send_message(user_id, text= all_text.get("локация").get(language),
                         reply_markup=bt.location_bt(user_id))
    ...
    # если отправил контакт не по кнопке возвращаем в эту же функцию
        ...
        bot.send_message(user_id, text= all_text.get("ошибка").get(language),
                         reply_markup=bt.phone_number_bt(user_id))
        bot.register_next_step_handler(message, get_phone, name,language)
...
def get_location(message, name, phone_number,language):
    ...
    bot.send_message(user_id, text= all_text.get("успеш_рег").get(language), reply_markup=bt.main_menu_bt(user_id))
...
def calls(call):
    ...
    if call.data == "back":
        ...
        bot.send_message(user_id, text=all_text.get("меню").get(language), reply_markup=bt.main_menu_bt(user_id))
    elif call.data == "plus":
        ...
        language = db.find_language_db(user_id)[0]
        bot.send_message(user_id, text=all_text.get("прод_усп").get(language), reply_markup=bt.product_in(all_products,user_id))
    elif call.data=="back_product":
        bot.delete_message(user_id, call.message.message_id)
        all_products = db.get_pr_id_name()
        language = db.find_language_db(user_id)[0]
        bot.send_message(user_id, text=all_text.get("выберите_п").get(language), reply_markup=bt.product_in(all_products,user_id))
    elif call.data == "cart":
        cart = db.get_user_cart(user_id)
        print(cart)
        language = db.find_language_db(user_id)[0]
        full_text =all_text.get("моя_к").get(language)
        count =0
        total_price = 0
        for product in cart:
            count +=1
        ...
        full_text += f"{all_text.get("сумма").get(language)} {total_price} sum"
        ...
        all_products = db.get_pr_id_name()
        language = db.find_language_db(user_id)[0]
        bot.send_message(user_id, text=all_text.get("кор_уже_очищ").get(language), reply_markup=bt.product_in(all_products,user_id))
    elif call.data == "order":
        cart = db.get_user_cart(user_id)
        print(cart)
        language = db.find_language_db(user_id)[0]
        full_text = f"{all_text.get("нов_заказ").get(language)}{user_id}:\n"
        count = 0
        total_price = 0
        for product in cart:
            count += 1
        ...
        bot.send_message(user_id, text= all_text.get("заказ_принят").get(language), reply_markup = bt.main_menu_bt(user_id))
...
def get_prod_info(call):
    ...
    bot.send_photo(chat_id=user_id, photo=product_info[3], caption=f"{product_info[0]}\n"
                                                                   f"{all_text.get("цена").get(language)} {product_info[1]} sum\n"
                                                                   f"{all_text.get("описание").get(language)} {product_info[2]}",
...
def text_handler(message):
    ...
    if message.text == "🍴Меню🍴" or message.text == "🍴Menu🍴":
        ...
        bot.send_message(user_id, text= all_text.get("отзыв_напишите").get(language))
    elif message.text == "🛒Корзина🛒" or message.text == "🛒Savatcha🛒":
        bot.send_message(user_id, text =all_text.get("моя_к").get(language))
